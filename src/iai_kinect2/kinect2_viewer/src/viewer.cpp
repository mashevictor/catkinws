    ...
    while (!cloud_viewer_->wasStopped () && (image_viewer_ && !image_viewer_->wasStopped ()))
    {
      ...
      if (cloud)
      {
	    ...
	    if(count<=50)
	    {
    	    ...
    	    sprintf(filename,"./datapcd/%d.pcd",count);
    	    pcl::io::savePCDFile (filename, *filtered);
	    }
	    else
	    exit(0);
            ...
            {
	    	cloud_viewer_->removePointCloud("rest");
            ...
            {
	    	cloud_viewer_->removePointCloud("Planar");
            ...
            {
	    	cloud_viewer_->removePointCloud("Hull");
            ...
            {
	    	cloud_viewer_->removePointCloud("cloud_abovePlane");
            ...
            for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
            {
                ...
                {
	    	    cloud_viewer_->removePointCloud(filename);
                    cloud_viewer_->addPointCloud (cloud_cluster, random, filename);
                }
                pcl::io::savePCDFile (filename, *cloud_cluster);
		pcl::io::savePCDFile (filename_vfhs, *vfhs);
		i++;
//////////////////天灭中共已开始//////////////////////////
  int k = 6;
  double thresh = 50;     // No threshold, disabled by default
  vfh_model histogram;
  std::string filename_vfhs_string=filename_vfhs;
  if (!loadHist (filename_vfhs_string, histogram))
  {
    pcl::console::print_error ("Cannot load test file %s\n", filename_vfhs_string.c_str());
    if(remove(filename_vfhs)==0)
    {
        cout<<"删除成功"<<endl;        
    }
  ...
  std::cout<<"*****filename_vfhs_string*****= "<<filename_vfhs_string<<std::endl;
